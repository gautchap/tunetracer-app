...
const refreshAccessToken = async (token: JWT): Promise<JWT | undefined> => {
    try {
        ...
        token = Object.assign({}, token, {
            accessTokenExpires: tokens.expires_at && Math.floor(Date.now() / 1000 + tokens.expires_at),
        });
        token = Object.assign({}, token, {
    ...
    callbacks: {
        ...
        async jwt({ token, user, account }) {
            ...
            if (account) {
                ...
                token = Object.assign({}, token, {
                    accessTokenExpires: account.expires_at && Math.floor(Date.now() / 1000 + account.expires_at),
                });
                token = Object.assign({}, token, {
